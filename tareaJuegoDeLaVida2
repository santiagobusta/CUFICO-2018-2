...
public:  //se van a definir datos publicos
  ...
  int vecinos1;  //define los vecionos como enteros
  int vecinos2;  //define los vecionos como enteros
  ...
  int analizarVecinos1(int posf, int posc); //funcion entera con dos entradas enteras llamada analizar vecinos
  int analizarVecinos2(int posf, int posc); //funcion entera con dos entradas enteras llamada analizar vecinos
  ...
  for(int f=0; f<fil; f++) //barre las filas de la red
    {
      for(int c=0; c<col; c++) //barre las columbas de la red
        {
	  mapa[f][c] = rand()%3; // para cada entrada en la red pone de manera aleatorea 0, 1 o 2
        }
    }
}
  ...
  for(int f=0; f<fil; f++)  //barre las filas de la red
    {
	  ...
	  if(mapa[f][c] == 1)  //si la entrada de la red es igual a 1
	    cout << "* ";      //pone un * 
	  if(mapa[f][c] == 2)  //si la entrada de la red es igual a x
	    cout << "x ";      //pone un x
	  if(mapa[f][c] == 0)  //sino
	    cout << ". ";      //pone un .
        }
...
int Mapa::analizarVecinos1(int posf, int posc) //se define la funcion analizarVecinos dentro de la clase Mapa
{
  vecinos1 = 0;  //se inicia la variable vecinos con un cero
  if(posf-1 >= 0 and posc-1 >= 0) //busca la entrada de la red que es vecino del punto estudado, ubicado en la esquina superior izquierda
    if(mapa[posf-1][posc-1] == 1) //y ademas si en esta ubicacion hay un uno
      vecinos1++;                  //a la variable vecino se le suma 1
  if(posf-1 >= 0)                 //busca la entrada de la red que es vecino del punto estudiado, ubicado arriba
    if(mapa[posf-1][posc] == 1)   //si en esta ubicacion hay un uno
      vecinos1++;                  //a la variable vecino se le suma uno
...
int Mapa::analizarVecinos2(int posf, int posc) //se define la funcion analizarVecinos dentro de la clase Mapa
{
  vecinos2 = 0;  //se inicia la variable vecinos con un cero
  if(posf-1 >= 0 and posc-1 >= 0) //busca la entrada de la red que es vecino del punto estudado, ubicado en la esquina superior izquierda
    if(mapa[posf-1][posc-1] == 2) //y ademas si en esta ubicacion hay un uno
      vecinos2++;                  //a la variable vecino se le suma 1
  if(posf-1 >= 0)                 //busca la entrada de la red que es vecino del punto estudiado, ubicado arriba
    if(mapa[posf-1][posc] == 2)   //si en esta ubicacion hay un uno
      vecinos2++;                  //a la variable vecino se le suma uno
  if(posf-1 >= 0 and posc+1 <= col-1) //busca la entrada de la red que es vecino del punto estudado, ubicado en la esquina superior derecha
    if(mapa[posf-1][posc+1] == 2)     //si en esta ubicacion hay un uno
      vecinos2++;                      //a la variable vecino se le suma uno
  if(posc-1 >= 0)                     //busca la entrada de la red que es vecino del punto estudiado, ubicado al lado izquierdo
    if(mapa[posf][posc-1] == 2)       //si en esta ubicacion hay un uno    
      vecinos2++;                      //a la variable vecino se le suma uno
  if(posc+1 <= col-1)                 //busca la entrada de la red que es vecino del punto estudiado, ubicado al lado derecho
    if(mapa[posf][posc+1] == 2)       //si en esta ubicacion hay un uno   
     vecinos2++;                       //a la variable vecino se le suma uno
  ...
  for (int f=0; f<fil; f++)  //barre todas las filas
    {
      for(int c=0; c<col; c++) //barre todas las columnas
        {
	  int n_vecinos1 = analizarVecinos1(f, c); //define un entero n_vecinos que es igual al numero que arroja la funcion analizarVecinos
	  int n_vecinos2 = analizarVecinos2(f, c); //define un entero n_vecinos que es igual al numero que arroja la funcion analizarVecinos
	  if(mapa[f][c] == 0) //si la entrada evaluada es igual a cero
            {
	      if(vecinos1 == 3 || vecinos2 == 3)//si ademas el numero de vecinos es tres
	  ...
	  if (mapa[f][c] == 1) //si la entrada evaluada es igual a uno
            {
	      if(vecinos1 == 2 || vecinos1 == 3 || vecinos2 == 2 || vecinos2 == 3)//si ademas el numero de vecinos es dos o tres
                 if(vecinos1>=vecinos2)  //y vecinos1 es mayor o igual a vecinos2
	            nueva_conf[f][c] = 1;  //cambie la entrada por un uno
                 else                    //sino
		   nueva_conf[f][c] = 2; //cambie por 2
	      else                     //sino
		nueva_conf[f][c] = 0;  //cambie la entrada por un cero
            }
          if (mapa[f][c] == 2) //si la entrada evaluada es igual a uno
            {
	      if(vecinos1 == 2 || vecinos1 == 3 || vecinos2 == 2 || vecinos2 == 3)//si ademas el numero de vecinos es dos o tres
                 if(vecinos1>=vecinos2)  //y vecinos 1 es mayor o igual que vecinos2
	            nueva_conf[f][c] = 1;  //cambie la entrada por un uno
                 else                    //sino
		   nueva_conf[f][c] = 2; //cambie la entrada por 2
	      else                     //sino
		nueva_conf[f][c] = 0;  //cambie la entrada por un cero
            }
        }
